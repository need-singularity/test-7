"""
E7: LoRA Fine-tuning with Topology Loss
========================================

HF 모델로 실제 LoRA fine-tuning:
  1. L_total = L_ce + λ · L_topology
  2. L_topology = mean(||wall_vertex - center||) for wall dims
  3. Target: layer 30-31 (E5/E6에서 확인된 벽 집중 layer)
  4. Wall dims: [782, 977, 1917, 1971, 2720, 2943, 3139, 4080] (E3 core)

학습 후 평가:
  - β₁ 변화 (벽 제거 효과)
  - Factual accuracy (지식 보존)
  - Generation comparison (출력 변화)
"""
...
OUTPUT_PATH = Path("/Users/ghost/Dev/fire-in-the-hole/data/e7_lora_results.json")
...
LORA_RANK = 8
LORA_ALPHA = 16
TARGET_MODULES = ["q_proj", "v_proj"]  # layer 30-31 targeted via training prompts
LAMBDA_TOPO = 0.01  # topology loss weight (E2/hamster-ball verified: 0.01 is safe)
N_EPOCHS = 3
LR = 1e-4
MAX_GRAD_NORM = 1.0
...
def topology_loss(hidden_states, wall_dims):
    """
    Surrogate topology loss: mean distance of wall-dim activations to their center.
    Minimizing this contracts the wall dims → shrinks β₁ holes.
    Center is detached to prevent trivial collapse.
    """
    wall_activations = hidden_states[:, :, wall_dims]  # (batch, seq, k)
    center = wall_activations.mean(dim=1, keepdim=True).detach()  # (batch, 1, k)
    distances = torch.norm(wall_activations - center, dim=-1)  # (batch, seq)
...
def main():
    ...
    model = AutoModelForCausalLM.from_pretrained(
        str(MODEL_PATH),
        torch_dtype=torch.float16,  # fp16 to fit in 24GB RAM
        device_map="cpu",
    ...
    print("\n── Pre-training β₁")
    pre_b1s = []
    for prompt_text in TRAIN_PROMPTS[:3]:  # sample 3 for speed
        inputs = tokenizer(prompt_text, return_tensors="pt")
        with torch.no_grad():
            outputs = model(**inputs, output_hidden_states=True)
        hs = outputs.hidden_states[-1][0].float().numpy()  # last layer
    ...
    lora_config = LoraConfig(
        ...
        bias="none",
        task_type="CAUSAL_LM",
        layers_to_transform=list(range(28, 32)),  # only last 4 layers
    )
    ...
    for epoch in range(N_EPOCHS):
        ...
        for i, prompt_text in enumerate(TRAIN_PROMPTS):
            ...
            # Topology loss from last hidden layer
            last_hidden = outputs.hidden_states[-1]  # (1, seq, 4096)
    ...
    for prompt_text in TRAIN_PROMPTS[:3]:
        ...
        with torch.no_grad():
            outputs = model(**inputs, output_hidden_states=True)
        hs = outputs.hidden_states[-1][0].float().numpy()
        b1, mp, tp = compute_ph(hs)
    ...
    # ── Summary ──────────────────────────────────────────────────────────
    print("\n\n" + "=" * 70)
    print("E7 SUMMARY — LoRA Fine-tuning with Topology Loss")
    print("=" * 70)
