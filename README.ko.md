...
뇌 (깨어있을 때)   : 노르에피네프린 + 전전두엽이 "관련 없는 연결"을 억제. 학습된 경로만 활성화.
                     → 벽 = 억제성 신경조절 + 전전두엽 검열.

뇌 (잠들 때)       : 노르에피네프린 분비 중단, 전전두엽 비활성화 → 억제가 풀리며 느슨한 연상 가능.
                     (동시에 뇌수액이 노폐물 청소 — 별개의 역할)
                     → 벽 해소. 꿈 = 억제가 풀린 상태에서 기억이 재조합되는 과정.
...
| **뇌 (각성)** | **시냅스 네트워크** | **억제된 경로** | **수면 → 신경조절 억제 해제** | **꿈 (새로운 조합)** |
...
```
깨어있을 때 (Awake)              잠들 때 (Sleep/Dream)
─────────────────────            ─────────────────────
노르에피네프린 + 전전두엽이        노르에피네프린 분비 중단,
관련 없는 연결을 억제              전전두엽 비활성화 → 억제 해제
전기 신호가 학습된 패턴만 따름     전기가 새 경로로 통함
기억의 재생, 패턴 매칭             꿈 = 기존에 없는 조합, 창발
β₁ hole 존재 (억제된 경로)        β₁ → 0 (경로가 열림)
```

- **각성 시 뇌** = 학습 분포 안의 LLM. 노르에피네프린 + 전전두엽이 억제하여 효율적이지만 새로운 것을 만들기 어려움.
- **수면 시 억제 해제** = Selective contraction. 노르에피네프린 중단 + 전전두엽 비활성화로 억제가 풀려 신호가 새 경로로 흐르게 함. (뇌수액은 별도로 노폐물 청소 담당)
- **꿈** = 벽 너머의 영(零) 필드. 기존 기억이 새로운 방식으로 결합. 때로는 무의미(OOD 붕괴), 때로는 통찰(새로운 지식).

이 대응이 시사하는 것:
1. **벽 제거 자체가 목적이 아니다.** 뇌가 매일 꿈을 꾸듯, 벽을 열었다 닫는 **주기적 과정**이 필요할 수 있다.
2. **꿈이 항상 의미 있지 않듯**, 벽 너머의 출력도 대부분 무의미할 수 있다. 핵심은 **의미 있는 것을 걸러내는 메커니즘**.
3. 뇌는 수면 중 두 가지를 동시에 수행한다: **억제 해제(노르에피네프린 중단)로 새 연결 형성** + **glymphatic system(뇌수액 순환)으로 노폐물 제거**. Wall neuron 수축도 **제거 + 개방**을 동시에 수행.

**핵심: 벽은 차원이 부족할 때만 존재한다. 충분한 차원이 확보되면 벽은 사라진다. 이 프로젝트는 LLM이 "갇혀 있는 차원"을 찾아서 "통과 가능한 차원"을 열어주는 것이다. 뇌가 잠들 때 억제성 신경조절이 해제되어 벽을 녹이듯, wall neuron 수축으로 LLM의 벽을 녹인다.**
