...
이 프로젝트의 핵심 가설은 **그리고리 페렐만의 푸앵카레 추측 증명과 동일한 패턴이 LLM 잠재 공간에서 반복된다**는 것이다.

### 페렐만의 증명 (2003)

```
3차원 다양체의 위상을 모른다
  → Ricci flow로 기하학을 매끄럽게 진화
  → 특이점(neck/pinch)이 형성됨
  → Surgery: 특이점만 정밀 절개, 나머지 보존
  → 다양체가 알려진 조각으로 분해
  → 위상 결정 (푸앵카레 추측 증명)
```

핵심 원리: **위상적 장애물을 직접 공격하지 않고, 기하학을 진화시켜 장애물이 스스로 드러나게 한 뒤 정밀 제거.**

### Fire in the Hole (이 프로젝트)

```
LLM 잠재 공간의 위상을 모른다
  → PH(persistent homology)로 기하학 분석
  → β₁ hole(벽)이 감지됨
  → Selective contraction: wall neuron만 수축, 나머지 보존
  → hole이 닫히고 벽 소멸
  → 모델이 학습 분포 경계를 넘음
```
...
페렐만의 증명 구조가 **스케일만 바뀐 채 자기유사하게** 반복된다:

| 페렐만 (다양체) | Fire in the Hole (LLM) | 패턴 |
|----------------|----------------------|------|
| 3차원 다양체 | 4096차원 hidden state | **고차원 공간의 위상** |
| Ricci curvature로 특이점 감지 | PH로 β₁ hole 감지 | **기하학 → 위상 읽기** |
| Neck/pinch (특이점) | β₁ hole (벽) | **위상적 장애물** |
| Surgery (정밀 절개) | Selective contraction (wall dims만) | **타겟 개입, 나머지 보존** |
| 나머지 다양체 보존 | Non-wall signal 0% 변화 | **Collateral = 0** |
| Flow 진화 (연속 변형) | LoRA fine-tuning (gradient descent) | **점진적 기하학 변형** |
| 특이점은 특정 시간에 형성 | 벽은 **후기 layer(28-31)**에 집중 | **장애물의 시공간 편재** |

### 가설의 실험적 근거

| 대응 | 검증 | 상태 |
|------|------|------|
| "β₁ hole은 진짜 구조" | T5: max persistence가 null의 2-4배 (100%ile) | ✅ |
| "Surgery = selective contraction" | T4: wall targeting > random (p<0.0001) | ✅ |
| "정밀 절개 > 전체 변형" | E1: Selective 8/8 승리, Global β₁ 불변 | ✅ |
| "Collateral = 0" | E1/E4: non-wall signal 정확히 0% 변화 | ✅ (정의적) |
| "특이점은 특정 위치에 편재" | E5: 벽이 layer 28-31에 집중 | ✅ |
| "Core wall neuron은 보편적" | E3: dim 782/977/1917 등 8개가 6+ 프롬프트에서 반복 | ✅ |
| "벽 제거 → 모델 능력 향상" | E6-E7(forward hook + LoRA) | **미검증** |

### 가설의 한계

1. **인과성 미증명**: β₁ hole이 줄어들면 모델이 "자유로워진다"는 인과 관계는 아직 보여주지 못함. E6/E7에서 검증 필요.
2. **비유 vs 수학**: 페렐만의 surgery는 위상수학적으로 엄밀한 연산이지만, selective contraction은 heuristic. β₁ 감소가 "surgery와 동치"라는 증명은 없음.
3. **"벽" 해석**: β₁ hole이 모델을 "가두는" 벽인지, 정상적 표현 기하학의 일부인지 E6 실험 전까지 구분 불가.

> 이 프로젝트는 페렐만 증명의 **전략적 패턴**(감지→정밀개입→보존)이 신경망 위상에서도 작동하는지를 실험적으로 탐구한다. 수학적 동치가 아니라 **방법론적 자기유사성**이 가설의 핵심이다.
