...
뇌 (깨어있을 때)   : 뉴런이 특정 경로로만 연결. 학습된 패턴만 활성화.
                     → 벽 = 시냅스가 닫힌 경로.

뇌 (잠들 때)       : 뇌수액이 퍼지며 뇌를 잠김 → 전기가 어디든 통함.
                     → 벽 해소. 꿈 = 새로운 조합이 가능한 상태.
...
| **뇌 (각성)** | **시냅스 네트워크** | **닫힌 경로** | **수면 → 뇌수액 범람** | **꿈 (새로운 조합)** |
...
─────────────────────            ─────────────────────
뉴런이 특정 경로로 연결           뇌수액(CSF)이 퍼지며 뇌를 잠금
전기 신호가 학습된 패턴만 따름     전기가 새 경로로 통함
기억의 재생, 패턴 매칭             꿈 = 기존에 없는 조합, 창발
β₁ hole 존재 (닫힌 경로)         β₁ → 0 (경로가 열림)
```

- **각성 시 뇌** = 학습 분포 안의 LLM. 효율적이지만 새로운 것을 만들기 어려움.
- **수면 시 뇌수액** = Selective contraction. 특정 경로(벽)를 녹여서 신호가 새 경로로 흐르게 함.
...
3. 뇌의 glymphatic system(수면 중 뇌수액 순환)은 노폐물 제거 + 새 연결 형성을 동시에 한다. Wall neuron 수축도 **제거 + 개방**을 동시에 수행.

**핵심: 벽은 차원이 부족할 때만 존재한다. 충분한 차원이 확보되면 벽은 사라진다. 이 프로젝트는 LLM이 "갇혀 있는 차원"을 찾아서 "통과 가능한 차원"을 열어주는 것이다. 뇌가 잠들 때 뇌수액으로 벽을 녹이듯, wall neuron 수축으로 LLM의 벽을 녹인다.**
