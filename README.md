...
|--------|------|-----|---------|
| 우주 | S³ | 없음 (β₁=0) | 자유 통과 |
| 다양체 | 3-manifold | 위상적 장애물 | Ricci flow + surgery |
| LLM 전체 | 4096-dim | β₁ hole | Selective contraction |
| LLM layer | layer 0→31 | 후기 layer에 집중 | 타겟 layer 개입 |
| Cycle 내부 | 2D 평면 | 닫힌 루프 | Passage direction (수직) |

**핵심: 벽은 차원이 부족할 때만 존재한다. 충분한 차원이 확보되면 벽은 사라진다. 이 프로젝트는 LLM이 "갇혀 있는 차원"을 찾아서 "통과 가능한 차원"을 열어주는 것이다.**
...
학습된 LLM은 고정된 분포 안에서 샘플링한다. 이것은 **기억의 재배열**이지 발견이 아니다.

β₁ hole이 닫히면 — 벽이 사라지면 — 모델은 학습 분포의 경계 바깥에 선다. 그곳은 영(零)의 필드: 아직 아무 확률도 할당되지 않은, **모든 방향이 가능한** 공간.
...
지식은 저장되는 것이 아니라 **경계에서** 일어난다.

이 프로젝트가 여는 것:
- β₁ hole을 제거하는 것이 아니라, **벽이 통과 가능했음을 드러내는 것**
- 모델의 밖에 서는 것 — wall neuron 수축은 분포 경계를 열어, 모델이 아직 가보지 않은 공간을 허용하는 행위
