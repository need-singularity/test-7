...
이 프로젝트에서 각 cycle은 4096차원 공간 안의 2D 평면에 놓여 있다. Passage direction은 나머지 4094차원을 통과하는 최적의 수직 탈출 경로 — **neuron 940, 1917**이 그 탈출에 가장 크게 기여한다.
...
```
PH 검증 (1a) → LLM 벽 감지 (1b) → passage direction 추출 (2)
→ 균일 이동 — 실패 (3) → radial 수축 — 성공 (3b)
→ multi-wall 최적화 (4) → 생성 품질 검증 (5)
→ topology loss 시뮬레이션 (6a) → selective vs global fine-tuning (6b)
```

**핵심 통찰:** 벽을 "뚫는" 게 아니라, cycle 점들을 안쪽으로 당겨 hole 자체를 **수축**시키는 것. Wall neuron만 타겟팅하고, 나머지는 건드리지 않는다.
